...
/*   Updated: 2022/05/20 17:37:38 by agarzon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executForm(const Form &form){
	if (this->grade < form.getGradeToExecute() && form.getSign()){
		std::cout << *this << " execute " << form << std::endl;
	}
	else if (!form.getSign())
		std::cout << this->name << " can't execute " << form.getName()
					<< " because this form are not signed" << std::endl;
	else
		std::cout << this->name << " can't execute " << form.getName()
					<< " because this form is execute grade " << form.getGradeToSign()
					<< std::endl;
